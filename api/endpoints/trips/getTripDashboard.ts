...
export const getTripDashboard: FastifyPluginAsync = async (fastify) => {
  fastify.get<{ Params: TripIdParams }>(
    '/trips/:tripId/dashboard',
    async (request, reply): Promise<TripDashboard | void> => {
      ...
      try {
        ...
        request.log.error(
          ...
          'Failed to retrieve trip dashboard'
        );

        return reply.code(500).send({
          message: 'Failed to retrieve trip dashboard'
        });
      }
    }
  );
